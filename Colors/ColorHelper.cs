...
public static class ColorHelper {
    private static readonly Regex s_RgbRegex = new(@"^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$", RegexOptions.Compiled);
    private static readonly Regex s_RgbaRegex = new(@"^rgba\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$", RegexOptions.Compiled);
    private static readonly Regex s_HsvRegex = new(@"^hsv\((\d{1,3}(?:\.\d+)?),\s*(\d{1,3}(?:\.\d+)?)(%?),\s*(\d{1,3}(?:\.\d+)?)(%?)\)$", RegexOptions.Compiled);
    private static readonly Regex s_HsvaRegex = new(@"^hsv\((\d{1,3}(?:\.\d+)?),\s*(\d{1,3}(?:\.\d+)?)(%?),\s*(\d{1,3}(?:\.\d+)?)(%?),\s*(\d{1,3}(?:\.\d+)?)(%?)\)$", RegexOptions.Compiled);
    private static readonly Regex s_HslRegex = new(@"^hsl\((\d{1,3}(?:\.\d+)?),\s*(\d{1,3}(?:\.\d+)?)(%?),\s*(\d{1,3}(?:\.\d+)?)(%?)\)$", RegexOptions.Compiled);
    private static readonly Regex s_HslaRegex = new(@"^hsl\((\d{1,3}(?:\.\d+)?),\s*(\d{1,3}(?:\.\d+)?)(%?),\s*(\d{1,3}(?:\.\d+)?)(%?),\s*(\d{1,3}(?:\.\d+)?)(%?)\)$", RegexOptions.Compiled);
    private static readonly Regex s_Hex3Regex = new(@"^#([a-fA-F0-9]{1})([a-fA-F0-9]{1})([a-fA-F0-9]{1})$", RegexOptions.Compiled);
    private static readonly Regex s_Hex4Regex = new(@"^#([a-fA-F0-9]{1})([a-fA-F0-9]{1})([a-fA-F0-9]{1})([a-fA-F0-9]{1})$", RegexOptions.Compiled);
    private static readonly Regex s_Hex6Regex = new(@"^#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})$", RegexOptions.Compiled);
    private static readonly Regex s_Hex8Regex = new(@"^#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})$", RegexOptions.Compiled);
    ...
    private static Color ExtractRgb(string query) {
        var match = s_RgbRegex.Match(query);
    ...
    private static Color ExtractRgba(string query) {
        var match = s_RgbaRegex.Match(query);
    ...
    private static Color ExtractHsv(string query) {
        var match = s_HsvRegex.Match(query);
        float h, s, v;

        h = Mathf.Clamp(float.Parse(match.Groups[1].Value), 0.0f, 360.0f) / 360f;
        var isSaturationPercentage = match.Groups[3].Value == "%";
        var isValuePercentage = match.Groups[5].Value == "%";
    ...
    private static Color ExtractHsva(string query) {
        var match = s_HsvaRegex.Match(query);
        float h, s, v, a;

        h = Mathf.Clamp(float.Parse(match.Groups[1].Value), 0.0f, 360.0f) / 360f;
        var isSaturationPercentage = match.Groups[3].Value == "%";
        var isValuePercentage = match.Groups[5].Value == "%";
        var isAlphaPercentage = match.Groups[7].Value == "%";
    ...
    private static Color ExtractHsl(string query) {
        var match = s_HslRegex.Match(query);
        float h, s, l;

        h = Mathf.Clamp(float.Parse(match.Groups[1].Value), 0.0f, 360.0f) / 360f;
        var isSaturationPercentage = match.Groups[3].Value == "%";
        var isLightnessPercentage = match.Groups[5].Value == "%";
    ...
    private static Color ExtractHsla(string query) {
        var match = s_HslaRegex.Match(query);
        float h, s, l, a;
