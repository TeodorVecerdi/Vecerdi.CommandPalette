...
using Vecerdi.CommandPalette.Basic.Views;
...
public partial class CommandsPlugin : IPlugin, IResourcePathProvider {
    ...
    private const string ParameterSuffixTexturePath = "Textures/right-chevron.png";
    ...
    public IEnumerable<ResultEntry> GetResults(Query query) {
        if (string.IsNullOrWhiteSpace(query.Text)) {
            ...
            foreach (var entry in CommandPaletteDriver.CommandEntries) {
                ...
                if (count >= MainView.MaxItemCount) {
                    break;
                }
            }
        ...
        var resultsDisplayName = CommandPaletteScorer.ScoreResults(query.Text, s_Settings.SearchCutoff, validCommands.Select(entry => entry.DisplayName));
        var resultsShortName = CommandPaletteScorer.ScoreResults(query.Text, s_Settings.SearchCutoff, validCommands.Select(entry => entry.ShortName));
        var resultDictionary = resultsDisplayName.ToDictionary(extractedResult => extractedResult.Index);

        foreach (var extractedResult in resultsShortName) {
    ...
    private ResultEntry CommandToResult(CommandEntry commandEntry, int score) {
        return new ResultEntry(
            new ResultDisplaySettings(commandEntry.DisplayName, commandEntry.ShortName, commandEntry.Description, commandEntry.Icon,
                commandEntry.HasParameters ? IconResource.FromResource(ParameterSuffixTexturePath) : default), score,
